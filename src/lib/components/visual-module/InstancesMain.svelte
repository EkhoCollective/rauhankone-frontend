<script lang="ts">
	...
	function populateFromData() {
		...
		for (let i = 0; i < data.clusters.length; i += 1) {
			...
			// Create curve points from all stories in this cluster
			const curvePoints: Vector3[] = [];

			// First, collect all story points for this cluster
			...
			// Add cluster SOM as the final point
			curvePoints.push(
				new Vector3(
					cluster.som[0] * worldScale,
					cluster.som[1] * worldScale,
					cluster.som[2] * worldScale
				)
			);

			// Create the curve with all points (need at least 2 points for a curve)
			// let clusterCurve: Vector3[] = [];
			// if (curvePoints.length >= 2) {
			// const catmul = new CatmullRomCurve3(curvePoints);
			// 	clusterCurve = catmul.getPoints(100);
			// } else {
			// 	// Fallback for clusters with only 1 story
			// 	clusterCurve = curvePoints;
			// }
